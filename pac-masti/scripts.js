...
function preload() {
  ...
  this.load.image('pill', pillPath);
  this.load.image('lifecounter', lifecounterPath);
  // this.load.image(
  //   'pill',
  //   'https://raw.githubusercontent.com/kudchikarsk/phaser-pacman/master/assets/images/pac%20man%20pill/spr_pill_0.png'
  // );
...
function create() {
  ...
  var font = '35px Arial semibold';
  var leftButton = this.add
    .text(160, 590, '   LEFT  ', { fill: 'white', font: font })
    .setInteractive()
    .on('pointerdown', () =>
      updateClickVirtualKeyboard(Phaser.Input.Keyboard.KeyCodes.A)
    );
  leftButton.setBackgroundColor('blue');

  var upButton = this.add
    .text(300, 550, '      UP     ', { fill: 'white', font: font })
    .setInteractive()
    .on('pointerdown', () =>
      updateClickVirtualKeyboard(Phaser.Input.Keyboard.KeyCodes.W)
    );
  upButton.setBackgroundColor('blue');

  var downButton = this.add
    .text(300, 590, '  DOWN  ', { fill: 'white', font: font })
    .setInteractive()
    .on('pointerdown', () =>
      updateClickVirtualKeyboard(Phaser.Input.Keyboard.KeyCodes.S)
    );
  downButton.setBackgroundColor('blue');

  var rightButton = this.add
    .text(450, 590, '  RIGHT  ', { fill: 'white', font: font })
    .setInteractive()
    .on('pointerdown', () =>
      updateClickVirtualKeyboard(Phaser.Input.Keyboard.KeyCodes.D)
    );
  rightButton.setBackgroundColor('blue');
...
function updateClickVirtualKeyboard(event) {
  keyA.isDown = false;
  keyW.isDown = false;
  keyS.isDown = false;
  keyD.isDown = false;
...
function resize() {
  ...
  console.log(canvas, windowWidth, gameRatio);
