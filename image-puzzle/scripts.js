$(document).ready(function () {
  ...
  $('.me').css({ 'background-image': 'url(' + imagesSfondo + ')' });
  ...
  function randomTile() {
    ...
    for (i = 0; i < orginal.length; i++) {
      box.append(
        '<div  class="me item_puzzle me_' +
          x[i] +
          ' tile" data-bid="' +
          x[i] +
          '"></div>'
      );
      if ((i + 1) % 6 == 0) box.append('<br class="item_puzzle">');
    }
    i = 17;
    return 0;
  }
  ...
  function changeBG(img) {
    document.getElementById('numero-immagine').innerHTML = `Puzzle ${img + 1}/${
      images.length
    }`;
  ...
  function nextImage(valore) {
    imgCount = imgCount + valore;
    if (imgCount < 0) {
      imgCount = 0;
    }
